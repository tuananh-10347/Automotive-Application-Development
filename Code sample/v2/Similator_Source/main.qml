...
Window {
    ...
    function saveData() {
        climate.setData(driverTemp.realValue,passengerTemp.realValue, outsideTemp.realValue,fanLevel.value,driver_wind_mode.currentIndex,passenger_wind_mode.currentIndex,auto_mode.currentIndex,sync_mode.currentIndex)
    }
    ...
    SpinBox {
        ...
        validator: DoubleValidator {
            bottom: Math.min(passengerTemp.from, passengerTemp.to)
            top:  Math.max(passengerTemp.from, passengerTemp.to)
        }

        textFromValue: function(value, locale) {
            return Number(value / 10).toLocaleString(locale, 'f', passengerTemp.decimals)
        }
    ...
    SpinBox {
        id: outsideTemp
        anchors.left: passengerTemp.left
        anchors.verticalCenter: temp3.verticalCenter
        from: 0
        to: 1000
        stepSize: 5
        ...
        validator: DoubleValidator {
            bottom: Math.min(outsideTemp.from, outsideTemp.to)
            top:  Math.max(outsideTemp.from, outsideTemp.to)
        }

        textFromValue: function(value, locale) {
            return Number(value / 10).toLocaleString(locale, 'f', outsideTemp.decimals)
        }
    ...
    Text {
        id: fan
        anchors.top: temp3.bottom
        anchors.topMargin: 30
        anchors.left: temp3.left
        text: "Fan: "
        horizontalAlignment: Text.AlignHCenter
    }
