...
double Climate::getTemp_outside()
{
    return m_outside_temp;
}

void Climate::setData(double temp_driver, double temp_passenger,double temp_outside, int fan_speed, int driver_wind_mode, int passenger_wind_mode, int auto_mode, int sync_mode)
{
    m_driver_temp = temp_driver;
    m_passenger_temp = temp_passenger;
    m_outside_temp = temp_outside;
